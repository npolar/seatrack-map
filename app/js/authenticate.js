...
export default () => {
    ...
    downloadDialog.querySelector('.seatrack-close').addEventListener('click', () => closeDialog());
    ...
    loginBtn.addEventListener('click', () => webAuth.authorize({ mode: 'login' }));
    signupBtn.addEventListener('click', () => webAuth.authorize({ mode: 'signUp' }));
    ...
    const showFiles = (files, folder) => {
        ...
        files.forEach(file => {
            tr = tbody.append('tr');
            tr.append('td').attr('class', 'nowrap').html('<a href="' + folder + file.name + '">' + file.name + '</a>'); // .html('<a href="">.SHP</a><br /><a href="">.GeoJSON</a>');
        ...
        if (isAuthenticated()) {
            ...
            getApprovalStatus((isApproved, table, folder) => {
                if (isApproved) {
                    ...
                    cartoSQL.execute("SELECT name, description, type, category, size, _format AS format FROM {{table}} ORDER BY name", { table: table })
