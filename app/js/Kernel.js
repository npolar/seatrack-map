...
export const Kernel = L.Class.extend({
  ...
  options: {
    ...
    sort: {
      ...
      season(a, b) {
        const seasons = ["autumn", "winter", "spring"];
        a = seasons.indexOf(a);
        b = seasons.indexOf(b);
  ...
  _initLayout(parent) {
    ...
    this._header
      ...
      .attr(
        "class",
        "accordion-delete-btn mdl-button mdl-button--icon mdl-js-button mdl-js-ripple-effect"
      )
    ...
    this._speciesSelect = this.createSelectControl(
      this._container,
      "species",
      options.species,
      this.onSpeciesSelect.bind(this)
    );
  ...
  select(id, value) {
    ...
    if (!itemEl.empty()) {
      ...
      if (id === "period") {
        this._state.allYears = value.toLowerCase().includes(allYearsString);

        if (this._state.allYears) {
          this._state[id] = value.toLowerCase().replace(allYearsString, "");
        }
      }
  ...
  updateLayer(state) {
    if (!this._prevState) {
      ...
      if (
        state.species === this._prevState.species &&
        state.season === this._prevState.season &&
        state.period === this._prevState.period &&
        state.colony === this._prevState.colony
      ) {
        return; // No change
      }
      this._prevState = L.extend({}, state);
  ...
  showMetadata(state) {
    ...
    this._rows.forEach(row => {
      if (
        ...
        row.colony.toLowerCase() === state.colony
      ) {
        ...
        table.append("caption").text("Data used to produce map");
        ...
        if (row.individuals < 5 || row.days < 10) {
          const warning = div.append("div").attr("class", "seatrack-limited");
          ...
          warning.append("span").text("Low amount of data");
        }
      }
    });
  },
  ...
  showLegend() {
    ...
    const div = this._container.append("div").attr("class", "seatrack-legend");

    const table = div.append("table");

    table.append("caption").text("Legend");
