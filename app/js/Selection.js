...
export const Selection = L.Class.extend({
  ...
  initialize(map) {
    ...
    select("#seatrack-add-button").on("click", () => {
      this._kernelLayers.push(this.addKernelLayer());
    });

    this._kernelLayers.push(
      this.addKernelLayer({
        state: {
  ...
  addKernelLayer(options) {
    ...
    const kernelLayer = kernel(select("#seatrack-accordion"), options).addTo(
      this._map
    );
